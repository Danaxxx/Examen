...
from api.serializers import DonacionSerializer , productoSerializer , SuscripcionSerializer, MarcaSerializer, CategoriaSerializer, medioSerializer, UserSerializer
...
def apipago(request):
    if request.method ==  'GET':
        ...
        serializer = medioSerializer(pago , many=True)
        return Response(serializer.data)
    elif request.method ==  'POST':
        data = JSONParser().parse(request)
        serializer = medioSerializer(data = data)
        if serializer.os_valid():
            serializer.save()
        return Response(serializer.data, status = status.HTTP_201_CREATED)
    return Response(serializer.errors, status = status.HTTP_400_BAD_REQUEST)
